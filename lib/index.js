...
const setLogExpirationTimer = (logFilePath, keepDays) => {
  ...
  const checkDelete = () => {
    if (fs.existsSync(logFilePath)) {
      ...
      fileDate <= deleteDate ? deleteLogFile(logFilePath) : '';
    }
  };

  setInterval(checkDelete, 1000);
};
...
const createLogFile = (config) => {
  ...
  !fs.existsSync(config.path) ? fs.mkdirSync(config.path, { recursive: true }) : '';

  const stream = fs.createWriteStream(logFilePath, {
    flags: 'a',
    defaultEncoding: 'utf8',
    bufferSize: config.writeBuffer,
  });
...
const createLogger = (config) => {
  ...
  return logger(logFileStream, ['LOG', 'INFO', 'WARN', 'ERROR', 'CLOSE'], config);
};
